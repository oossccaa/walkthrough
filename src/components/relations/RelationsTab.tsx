...
import { RELATION_TYPE_LABEL } from '../../labels'
import { fmt, durationLabel } from '../../utils/dates'
import { relationsRepo } from '../../db/repo'
import type { RelationPerson, RelationType } from '../../types'

const TYPES: RelationType[] = ['family', 'friend', 'ex']

export function RelationsTab({ personId, items }: { personId: string; items: RelationPerson[] }) {
  ...
  return (
    <div className="space-y-3">
      {TYPES.map(type => {
        ...
        if (list.length === 0 && type !== 'family') return null
        return (
                  ...
                  <li key={r.id}>
                    <button onClick={() => setEditing(r)} className="w-full py-2.5 text-left first:pt-0 last:pb-0">
                      <div className="flex items-center gap-2">
                        ...
                        {r.role && (
                          <span className="rounded-full bg-neutral-100 px-2 py-0.5 text-[11px] text-neutral-500">{r.role}</span>
                        )}
      ...
      {editing && (
        <RelationSheet personId={personId} existing={editing === 'new' ? null : editing} onClose={() => setEditing(null)} />
      )}
    </div>
  )
}

function RelationSheet({ personId, existing, onClose }: {
  personId: string
  existing: RelationPerson | null
  onClose: () => void
}) {
  const [type, setType] = useState<RelationType>(existing?.type ?? 'friend')
  ...
  return (
    <BottomSheet open onClose={onClose} title={existing ? '編輯人物' : '新增人物'}>
      <div className="space-y-4">
        ...
        <Field label="名字">
          <input className={inputCls} value={name} onChange={e => setName(e.target.value)} placeholder="例:Peggy" />
        </Field>
        <Field label="身分(選填)">
          <input className={inputCls} value={role} onChange={e => setRole(e.target.value)} placeholder="例:媽媽、閨蜜、大學同學" />
        </Field>
        {type === 'ex' && (
        ...
        <Field label="特徵備註(選填,見面前速查用)">
          <input className={inputCls} value={traits} onChange={e => setTraits(e.target.value)} placeholder="例:很重視禮貌,見面要帶伴手禮" />
        </Field>
        <Field label="地雷/注意事項(選填,會以紅字顯示)">
          <input className={inputCls} value={note} onChange={e => setNote(e.target.value)} placeholder="例:別提登山社" />
        </Field>
